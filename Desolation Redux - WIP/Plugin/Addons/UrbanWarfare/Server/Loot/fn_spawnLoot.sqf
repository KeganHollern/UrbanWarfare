...
_cfgLootTypes = configFile >> "CfgLoot" >> _buildingType;
...
_minPiles = getNumber(configFile >> "CfgLoot" >> "minPiles");
		...
		for "_i" from 1 to (floor(random(3)) + 1) do {
			...
			while{_selectingLoot} do {
				...
				if(_index == -1) then {
					...
					if(_count < getNumber(configFile >> "CfgLoot" >> "Limits" >> ("max" + _gearType))) then {
						_selectingLoot = false;
			...
			_lootList = getArray(configFile >> "CfgLoot" >> "LootTypes" >> _value >> _gearType);
