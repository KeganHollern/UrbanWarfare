...
} forEach getArray(configFile >> "CfgLoot" >> "BuildingTypes" >> "military");
{
	ind_positions pushBack (_x select 1);
	ind_buildings pushBack toLower(_x select 0);
} forEach getArray(configFile >> "CfgLoot" >> "BuildingTypes" >> "industrial");
{
	res_positions pushBack (_x select 1);
	res_buildings pushBack toLower(_x select 0);
} forEach getArray(configFile >> "CfgLoot" >> "BuildingTypes" >> "residential");
